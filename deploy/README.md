...
### Ротация учёток приложения из админ-панели (ADR-0011)

Пароль роли Postgres и ключ MinIO, под которыми работает приложение, меняются **без простоя** из
админ-панели: «Безопасность → Учётки БД/MinIO». Ротация всегда в три шага:

1. **«Сгенерировать новые»** (отдельно для PostgreSQL и MinIO). Панель выпускает учётку в запасном слоте
   (соседняя роль `familyhub_app_a` ↔ `familyhub_app_b`; новый service account MinIO) и **один раз**
   показывает строки для `PROD_ENV` — скопируйте и отметьте «Я сохранил». Закрыть панель, не сохранив
   значения, она не даст без подтверждения: показать их второй раз нельзя, они нигде не хранятся (в БД
   остаётся только история — кто, когда, из какой в какую учётку).
2. **Положите значения в `PROD_ENV`** (`DB_APP_USER` + `DB_APP_PASSWORD`, либо `MINIO_APP_ACCESS_KEY` +
   `MINIO_APP_SECRET_KEY`) и **задеплойте** (§6). Пока вы не задеплоили, приложение работает под старой
   учёткой — выпущенная новая никому не мешает.
3. **Вернитесь в панель.** Она сама заметит, что приложение работает под новой учёткой (по фактическому
   подключению, а не по клику), и появится кнопка **«Отозвать старую»**. Нажимайте её, когда убедитесь, что
   `/health/ready` зелёный: старая роль отключается, её открытые соединения обрываются, старый ключ MinIO
   удаляется. Новую ротацию можно начать только после отзыва старой.

Панель не даст: отозвать учётку, под которой приложение работает прямо сейчас; отозвать старую до того, как
новая применена; начать ротацию, пока не отозвана предыдущая. Потеряли выпущенные значения до деплоя —
просто сгенерируйте заново (прежняя неприменённая учётка заменяется, неиспользованный ключ MinIO удаляется).
Если приложение работает под суперпользователем/root (dev, либо не выполнена первичная настройка), панель
показывает предупреждение и ротацию не предлагает.

⚠️ После первой ротации **не запускайте `bootstrap-app-credentials.sh --force`** — он выпустит пароль для
`familyhub_app_a` и напечатает `DB_APP_USER=familyhub_app_a`, а приложение к этому моменту может работать
под `familyhub_app_b`. Меняйте учётки только из панели.
