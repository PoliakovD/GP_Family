...
/// LabAnalyteNormalizer.Normalize с уже извлечёнными показателями, см. MedicalDocumentExtractionProcessor).
/// Confidence — своя, СЕКЦИОННАЯ уверенность модели (не наследуется от документа целиком и не
/// подделывается кодом в 1.0, см. class doc MedicalDocumentExtractionProcessor.ResolveSpecimenKbIdAsync
/// про баг, который к этому привёл): секция с придуманным заголовком панели вместо реального
/// источника обязана пройти тот же порог MinConfidence, что и документ целиком.</summary>
public record SpecimenSection(string? Context, double Confidence, IReadOnlyList<string> IndicatorNames);
    ...
    ILmStudioJsonClient client, GlobalSpecimenKbService specimenKb, IPromptProvider promptProvider, ILogger<SpecimenResolver> logger)
{
    ...
    private const string SystemPrompt = """
        ...
        Формат ответа:
        {
          "context": "нормализованное название источника (например, \"кровь\", \"моча\", \"эякулят\", \"ЭКГ\") или null, если не удалось определить",
          ...
          "sections": [
            {
              "context": "источник этой секции — ТО ЖЕ САМОЕ понятие, что и context верхнего уровня (см. правила ниже)",
              "confidence": 0.0,
        ...
        - "context" (и верхнего уровня, и внутри каждой секции) — КОРОТКОЕ литературное название
          ТОЛЬКО самого источника/биоматериала/исследования: "кровь", "моча", "эякулят", "ЭКГ".
          НИКОГДА не включай в "context" название раздела бланка, вида исследования этого раздела
          или методики — "Физические свойства", "Микроскопическое исследование",
          "Биохимический анализ", "Общий анализ" и подобные слова ЭТО НЕ ИСТОЧНИК, а заголовок
          таблицы внутри источника — если видишь "Микроскопическое исследование эякулята", "context"
          должен быть "эякулят", а не вся фраза целиком. Одна и та же секция или разные секции
          ОДНОГО и того же бланка про один и тот же биоматериал/исследование должны получать ОДИНАКОВЫЙ
          "context" дословно — не варьируй формулировку между секциями одного источника.
        ...
        - "sections" — заполняй ТОЛЬКО если в этом фрагменте реально есть несколько групп
          ...
          "Общий анализ мочи" на одном бланке). Секции, описывающие один и тот же источник
          (например, "Физические свойства" и "Микроскопическое исследование" одного и того же
          эякулята) — это ОДНА секция с одним "context", не разные. Для обычного бланка с одним
          источником — пустой массив, всё определяется полем "context" верхнего уровня.
    ...
    private static List<SpecimenSection> ParseSections(Dictionary<string, JsonElement> payload)
    {
        ...
        foreach (var item in arr.EnumerateArray())
        {
            ...
            // Дефолт 0, не 1.0 — старый (ещё не обновлённый из админки) текст промпта не отдаёт
            // "confidence" на уровне секции вовсе; консервативный дефолт лучше, чем повторение
            // прежнего бага (жёсткая единица, обнулявшая порог/вето, см. class doc SpecimenSection).
            var confidence = ReadDouble(item, "confidence") ?? 0;
            ...
            sections.Add(new SpecimenSection(context, confidence, names));
        }
        return sections;
    }
}
