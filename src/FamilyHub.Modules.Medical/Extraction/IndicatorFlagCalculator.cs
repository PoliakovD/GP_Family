...
/// анализа, справочник даёт лишь общий ориентир. Пол пациента (identity rework: User.Gender/
/// FamilyDependent.Gender) здесь пока НЕ используется — справочник (GlobalLabAnalyteKb) даёт
/// только возрастные диапазоны, без разбиения по полу; подключение пола к референсным диапазонам
/// анализов — отдельная задача, вне объёма identity rework.
/// </summary>
