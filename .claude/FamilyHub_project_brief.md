...
- **Прямое личное назначение (`TargetUserId`)** — третий, независимый канал видимости в обход
  L1-шаринга: владелец назначает конкретную запись конкретному участнику общей семьи напрямую.
  В отличие от `FamilyMedicalShare`, это **постоянный грант**, НЕ завязанный на активное членство —
  переживает выход/исключение получателя из семьи (осознанное решение, не баг; см. class-doc
  `MedicalRecordService`). Если это когда-нибудь понадобится изменить — отзыв нужно добавить в
  `UserLeftFamilyMedicalCleanupConsumer` рядом с существующей зачисткой `FamilyMedicalShare`.
